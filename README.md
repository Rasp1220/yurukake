...
2. **ラウンドロビン方式で全都道府県に触れる。** 総合＋10ジャンルの11パターンのクエリを1つずつ「その回の担当ジャンル」として選び、まだ目標件数（`TARGET_VIDEOS_PER_PREFECTURE`＝700件）に届いていない都道府県**全員**にそのジャンルの1ページ目だけを取得する（＝1パス）。1パス終えたら、まだ届いていない県だけを対象に次のジャンルでもう1パス、を予算が尽きるかジャンルを一巡するまで繰り返す。予算8,000ユニット（80回の検索呼び出し）に対して都道府県は最大47なので、1パス目でほぼ全県に届き、残った予算は自然と「まだ目標に届いていない県」に厚く配分される（人気県は早く目標に届いてパス対象から外れるため）。すでに700件に達している県はYouTubeを一切呼ばずに飛ばす（レスポンスの `skippedAsSatisfied`）。予算切れで一度も触れられなかった県は次回の実行で最初から対象になる
